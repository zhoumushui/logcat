...
public class SuperUserHelper {
	...
	private static void showToast(final Context context, final int resId) { //用线程显示toast
	...
	private static List<Integer> getAllRelatedPids(final int pid) { //获得所有关联的pid
	...
	public static void destroy(Process process) {
	    ...
	    for (Integer relatedPid : allRelatedPids) { //匹配pid 并销毁
	        destroyPid(relatedPid);
	    }
        
	}
	
	private static void destroyPid(int pid) { //根据pid销毁 需要root
	...
	public static void requestRoot(Context context) { //判断有没有 root权限
